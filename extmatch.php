...
if ( $_REQUEST['frm'] == 'frmExtSearch' && !isset($_REQUEST['sort']) ){
	...
	$sqlselect = 'SELECT DISTINCT user.*, floor((to_days(curdate())-to_days(birth_date))/365.25)  as age FROM ! user INNER JOIN ! pref ON user.id=pref.userid JOIN '.MEMBERSHIP_TABLE.' as mem ON  user.level=mem.roleid and mem.includeinsearch = 1 ';
