...
if ( $searchtype == 'simple' ) {
	if ( $searchby == 'username' ) {
			...
			" as user WHERE status in ('active','".get_lang('status_enum','active')."') AND user.id > 0 AND user.id<>'" . $_SESSION['UserId'] .
		...
		$querystring = array('searchtype'=>'simple','searchby'=>$searchby,'txtsearch'=>$searchtxt);
			...
			" as user, " . USER_PREFERENCE_TABLE . " as pref where user.id=pref.userid  AND status in ('active','".get_lang('status_enum','active')."') AND user.id > 0 AND user.id<>'" . $_SESSION['UserId'] ."'";
			$keyval = '';
...
// . '%5B%5D=' .
	...
	if ( count($data) <= 0 ) {
		$t->assign ( 'error', "1" );
		$t->assign ( 'backlink', 'search_popup.php' );
		$t->assign ( 'lang', $lang );
...
//		$t->assign('rendered_page', $t->fetch('showsimpsh.tpl') );
	...
	$sql = 'SELECT DISTINCT user.*, floor((to_days(curdate())-to_days(birth_date))/365.25)  as age FROM ! as user WHERE user.id > 0 AND ';
